...
export default function ElencoLibretto() {
    const { librettoMap, numeriLibretto, loading, error } = useContext(FascicoloLibrettoContext);
    ...
    if (error) {
        return (
            <Container maxWidth="sm" sx={{ py: 5 }}>
                <Alert severity="error">Errore nel caricamento del libretto: {error}</Alert>
            </Container>
        );
    }

    // Filtriamo i canti in base alla ricerca
    const filteredNumeri = numeriLibretto.filter(numero => {
        const canto = librettoMap[numero];
        ...
        <Box sx={{ minHeight: '100vh', bgcolor: '#f8f5ec', pb: 8, pt: { xs: 3, sm: 5 } }}>
            <Container maxWidth="sm">
                ...
                <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', mb: 4 }}>
                    <Avatar sx={{ bgcolor: '#1976d2', width: 64, height: 64, mb: 2, boxShadow: 2 }}>
                ...
                <Paper elevation={0} sx={{ p: 1, mb: 4, borderRadius: 3, bgcolor: '#fffdf7', boxShadow: "0 4px 12px rgba(0,0,0,0.05)" }}>
                    <TextField
                        fullWidth
                        placeholder="Cerca per numero (es. L20) o titolo..."
                        variant="outlined"
                        value={searchTerm}
                ...
                <Paper elevation={0} sx={{ borderRadius: 3, overflow: 'hidden', bgcolor: '#fffdf7', boxShadow: "0 4px 12px rgba(0,0,0,0.05)" }}>
                    {filteredNumeri.length === 0 ? (
                        <Typography align="center" sx={{ py: 6, color: 'text.secondary', fontFamily: "Georgia, serif" }}>
                            Nessun canto trovato nel libretto.
                        </Typography>
                    ) : (
                        <List disablePadding>
                            {filteredNumeri.map((numero, index) => {
                                ...
                                return (
                                    <ListItem key={numero} disablePadding divider={index < filteredNumeri.length - 1}>
                                        ...
                                        >
                                            <Avatar sx={{ bgcolor: '#e3f2fd', color: '#1976d2', mr: 2, width: 55, height: 55, fontWeight: 'bold', fontSize: '1.1rem' }}>
                                                {canto.etichetta}
                                            </Avatar>
                                            <ListItemText
